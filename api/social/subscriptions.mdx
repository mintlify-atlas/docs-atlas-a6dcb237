...
description: "Toggle channel subscriptions, retrieve a channel's subscriber list, and get all channels the current user subscribes to."
---
...
Toggles the subscription state between the authenticated user and the given channel. If a subscription record already exists it is deleted and `isSubscribed: false` is returned. If no subscription exists a new record is created and `isSubscribed: true` is returned.
