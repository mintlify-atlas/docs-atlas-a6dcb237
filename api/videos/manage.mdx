...
Flips the `isPublished` flag on a video between `true` and `false`. Only the video's owner can call this endpoint — ownership is verified in the controller before the toggle is applied.
