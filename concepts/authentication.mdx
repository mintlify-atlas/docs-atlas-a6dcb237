...
On success, the server sets `accessToken` and `refreshToken` as `HttpOnly` cookies **and** returns them in the response body so that non-browser clients can store them explicitly:
...
A new pair of `accessToken` and `refreshToken` is issued and set as cookies. The old refresh token is invalidated in the database.
...
1. Clears the `refreshToken` field on the `User` document in MongoDB, invalidating the session server-side
