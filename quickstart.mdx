---
title: "Get Started with VideoHub in 5 Minutes"
...
The registration endpoint accepts a multipart form so it can receive an avatar image alongside the text fields. The `avatar` file field is required; `coverImage` is optional.
...
```json
{
  ...
  "data": {
    ...
    "fullName": "John Doe",
    "avatar": "https://res.cloudinary.com/your-cloud/image/upload/v.../avatar.jpg",
    "coverImage": "",
    "watchHistory": [],
...
1. **HTTP-only cookies** (`accessToken` and `refreshToken`) — consumed automatically by browsers
